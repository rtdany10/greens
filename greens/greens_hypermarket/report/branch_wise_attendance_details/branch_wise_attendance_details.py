		...
		for parameter in group_by_parameters:
			...
			if att_map_set & emp_map_set:
				parameter_row = ["<b>" + parameter + "</b>"] + [
					"" for day in range(filters["total_days_in_month"] + 3)
				]
...
def get_columns(filters):
	...
	columns += [_("Employee") + ":Link/Employee:120", _("Employee Name") + ":Data/:120", _("Employee Branch") + ":Link/Branch:120"]
	days = []
	for day in range(filters["total_days_in_month"]):
...
def get_employee_details(group_by, company, branch=None):
	...
	if branch:
		...
		branch = frappe.db.get_list("Branch", pluck="name")
		if len(branch) == 1:
			cond += " and branch = %s" % frappe.db.escape(branch[0])
		else:
			cond += " and branch IN {}".format(tuple(branch))
